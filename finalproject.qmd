...
```{python}
# make sure column names match for merge
trees_grouped_borough = trees_grouped_borough.rename(columns={"borough": "BoroName"})
```

```{python}
# merge data frames together
merged_borough_map = borough_data.merge(trees_grouped_borough, how='left', on='BoroName' )
```

```{python}
# create the choropleth
merged_borough_map.plot(column='2005', cmap='Greens',
                linewidth=0.1, edgecolor='0.8', legend=True)
